...
class YoloOnnxObjectDetection(YoloObjectDetection):
    ...
    def __init__(self, model_path: str,):
        ...
        self.EPs = ['CPUExecutionProvider']
        self.model = self._load_model(model_path)
        

        logging.info(f'Current Device: {self.EPs}')
    ...
    def preprocess(self, input) -> Tuple[np.ndarray, np.ndarray]:
        img_array = super().preprocess(input)

        im = np.stack(self.pre_transform([ img_array ]))
    ...
    def inference(self, source) -> Tuple[torch.Tensor, np.ndarray, np.ndarray]:
        preprocess_img, ori_img = source

        input_name = self.model.get_inputs()[0].name

        # Run inference
        result = self.model.run(None, {input_name: preprocess_img})

        return torch.from_numpy(result[0]), preprocess_img, ori_img
